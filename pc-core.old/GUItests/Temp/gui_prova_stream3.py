...
class ImageWidget(QtGui.QWidget):
	...
	def setImage(self, frm):
		if self.running:
			try:
				image = self.bridge.imgmsg_to_cv2(frm, "bgr8")
		...
		if self.ready:
			...
			try:
				...
				if self.image and self.running:
					...
					print("YES paintEvent "+str(self.index))
				'''
				else:
				...
				print("NO paintEvent "+str(self.index))
			self.ready = True
...
class Window(QtGui.QMainWindow,form_class):
	...
	def __init__(self,parent=None):
		...
		self.mainCam = ImageWidget(1, self.mainCam)
		self.mainCam.setSize(self.size.width(),self.size.height())

		self.cam2 = ImageWidget(2, self.cam2)
		self.cam2.setSize(self.size2.width(),self.size2.height())

		self.cam3 = ImageWidget(3, self.cam3)
...
class Converter:
	def __init__(self,window):
		...
		self.image_sub1 = rospy.Subscriber("/Cam0/webcam/image_raw",Image,self.callback1)
		self.image_sub2 = rospy.Subscriber("/Cam1/webcam/image_raw",Image,self.callback2)
		self.image_sub3 = rospy.Subscriber("/Cam2/webcam/image_raw",Image,self.callback3)

	def callback1(self,data):
		self.window.getCam(1).setImage(data)
		#print "sto settando"+str(self.count)


		#self.window.getCam(2).setImage(img,self.count)
		#self.window.getCam(3).setImage(img,self.count)

	def callback2(self,data):
		self.window.getCam(2).setImage(data)
		#print "sto settando"+str(self.count)


	def callback3(self,data):
		self.window.getCam(3).setImage(data)
		#print "sto settando"+str(self.count)
